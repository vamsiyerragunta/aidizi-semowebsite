...
class Jutecommissioner extends CI_Controller {
    ...
    public function Create(){ 
        ...
        $this->load->view('backend/Add_indent',$data);
        }
    else{
    ...
    public function GetSupplierByid(){
            $midbatch = $_POST['search'];
               ...
               $this->indent_model->GetSuppIDbatch($midbatch);
    }
    public function GetPurchaseparam(){
if ($this->session->userdata('user_login_access') != False) {
        $param = $_GET['param'];
        $purval = $this->indent_model->GetPurchaseparam($param);
    ...
    public function manageIndent(){
        if ($this->session->userdata('user_login_access') != False) {
        $data = array();
        $data['Purchase'] = $this->indent_model->getPurchaseInfo();        
        $this->load->view('backend/manage_indent',$data);
        } else {
            redirect(base_url(), 'refresh');
        } 
 }
    public function CreateMillOrder(){
        if ($this->session->userdata('user_login_access') != False) {
        $data = array();
        $data['Purchase'] = $this->indent_model->getPurchaseInfo();        
        $this->load->view('backend/CreateMillOrder',$data);
        } else {
            redirect(base_url(), 'refresh');
        }		
    }
    ...
    public function AddJuteMill(){
        ...
        $data = array();
        $data['Purchase'] = $this->indent_model->getPurchaseInfo();        
        $this->load->view('backend/AddJuteMill',$data);
        } else {
            redirect(base_url(), 'refresh');
        }		
    }
	public function ManageOrder(){
        ...
        $data = array();
        $data['Purchase'] = $this->indent_model->getPurchaseInfo();        
        $this->load->view('backend/ManageOrder',$data);
        } else {
            redirect(base_url(), 'refresh');
        }		
    }
    ...
    public function Purchase_Review(){
        ...
        if(!empty($bankid)){
            $bankname = $this->indent_model->GetBankName($bankid);
        }
        ...
        $invoiceid    = $this->indent_model->GePurchaseInvoice($invoice);
        if(!empty($invoiceid)){
    ...
    public function Save_Purchase(){
        ...
        $checksameinvoice = $this->indent_model->GePurchaseInvoice($invoice);
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $success = $this->indent_model->Save_Purchase($data);
            if($this->db->affected_rows()){
                ...
                $success = $this->indent_model->Insert_Supplier_amount($data);
                $data = array();
                $data = array(
                ...
                $success = $this->indent_model->Insert_Supplier_PayHistory($data);                 
                foreach($_POST['qty'] as $row=>$name){
                if(!empty($_POST['qty'][$row])){
                $medicine   =   $_POST['medicine'][$row];
                $qty        =   $_POST['qty'][$row];
                ...
                $success = $this->indent_model->Save_Purchase_History($data);
                    }
                }                
                foreach($_POST['qty'] as $row=>$name){
                if(!empty($_POST['qty'][$row])){
                ...
                $wholesaller=   $_POST['wholesaler'][$row];
                $expire     =   $_POST['expiredate'][$row];     
                //$medicinestock = $this->indent_model->getMedicineStock($medicine);
                //$instock = $medicinestock->instock + $qty;
                $medicinestock = $this->indent_model->getmedicineByMId($medicine);
                $instock = $medicinestock->instock + $qty;
                ...
                $success = $this->indent_model->Update_Medicine($medicine,$data);
                }
    ...
    public function Purchase_History(){
        ...
        $data = array();
        $data['purchasehistory'] = $this->indent_model->GetPurchaseHistory($id);
        $this->load->view('backend/purchase_history',$data);
    }
    public function GetPurchaseSpecificdata(){
        $pid = $this->input->get('id');
        $purchaseval = $this->indent_model->getPurchaseInvoiceData($pid);
        $indate = date('Y-m-d',$purchaseval->pur_date);
        $purchasedetails = $this->indent_model->getPurchaseDetailsInvoiceData($pid);
